...
**Status update 2026-07-18:** §1–§4 fixed in the references (CHANGELOG 26.29.6), with two scope notes: the staging tier (§3) stays out of scaffold scope as a product decision, and §4's "background-task errors invisible" finding was narrowed on verification — sentry-sdk's auto-enabled integrations and default logging integration already capture Celery/RQ/django-tasks failures, so the fix is a documentation note plus a post-deploy check. §5 closed with `references/conventions.md` (env var names, Redis DB map, compose service shape, SameSite rule, Python pins) plus conformance edits across the drifting files. All review sections are now addressed.
